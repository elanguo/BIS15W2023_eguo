...
```r
max(homerange_herbivores$mean.hra.m2, na.rm=T)
```
...
```r
max(homerange_carnivores$mean.hra.m2, na.rm=T)
```
...
```r
deer.old <- filter(homerange, family=="cervidae")
deer <- select(deer.old, "mean.mass.g", "log10.mass", "family", "genus", "species")
