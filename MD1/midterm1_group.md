...
```r
eagles_conservation_status<-eagles %>%
...
ecosphere%>%
  group_by(habitat) %>%
  summarize(min_egg=min(mean_eggs_per_clutch),
            max_egg=max(mean_eggs_per_clutch),
            mean_egg=mean(mean_eggs_per_clutch))
```

```
## # A tibble: 7 × 4
##   habitat   min_egg max_egg mean_egg
##   <chr>       <dbl>   <dbl>    <dbl>
## 1 Grassland     2.5    15       5.19
## 2 Ocean         1       5       2.22
## 3 Shrubland     2      17       4.74
## 4 Various       1      15.5     4.36
## 5 Wetland       1.5    14.5     5.29
## 6 Woodland      1      12.5     4.32
## 7 <NA>          2       6.5     3.61
```
